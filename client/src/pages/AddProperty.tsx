            ...
            <div className="bg-white rounded-2xl p-8 shadow-sm min-h-[300px]">
              ...
              {step === 4 && (
                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-4">
                  ...
                  <div><label className="block text-sm font-medium text-gray-700 mb-1.5">{isAr ? "رقم الجوال *" : "Phone *"}</label><input type="tel" required value={form.phone} onChange={e => setForm({ ...form, phone: e.target.value.replace(/[^0-9+\-\s()]/g, '') })} className="w-full px-4 py-3 bg-[#f8f5f0] rounded-lg focus:outline-none focus:ring-2 focus:ring-[#c8a45e]/30" placeholder="+966 5x xxx xxxx" dir="ltr" disabled={submitMutation.isPending} /><p className="text-xs text-gray-400 mt-1" dir="ltr">{isAr ? "مثال: 0512345678 أو +966512345678 أو أي رقم دولي" : "e.g. 0512345678, +966512345678, or any international number"}</p></div>
