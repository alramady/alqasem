 ...
 * On Manus: uses the forge notification service.
 * On Railway: logs to console (can be extended to email/Slack).
 * Returns `true` if handled, `false` if service unavailable.
 */
export async function notifyOwner(
  ...
  if (ENV.forgeApiUrl && ENV.forgeApiKey) {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        console.warn(`[Notification] Failed (${response.status}): ${detail}`);
        return false;
      }
      return true;
    } catch (error) {
  ...
  // Fallback: log to console (Railway deployment)
